...
def build_model_options(options, help_options, user_options, options_tree, fixed_params, architecture):
    ...
    #generator
    options_tree, fixed_params = build_generator_options(options, help_options, options_tree, fixed_params)
...
#generator
def build_generator_options(options, help_options, options_tree, fixed_params):
    if options['user_options']['generator']:
        generator, ground_station, options_tree = divide_options(options['user_options']['generator'], options_tree, help_options)
        options_tree, fixed_params = generator_overwrite(generator, help_options, options_tree, fixed_params)
        options_tree, fixed_params = generator_overwrite(ground_station, help_options, options_tree, fixed_params)
    else:
        options_tree.append(('model', 'generator', None, 'type', None, ('???', None),'x'))
    return options_tree, fixed_params
...
#    options_tree.append(('model', 'scaling', 'xd', 'v_sq', 5., ('descript', None), 'x'))
    ...
    ground_station['name'] = 'ground_station'
    generator['name'] = 'generator'

    return generator, ground_station, options_tree

def generator_overwrite(add_options, help_options, options_tree, fixed_params):
    options_type = add_options['name']
    for name in list(add_options.keys()):
        if name != 'name':
            if name in list(help_options['model'][options_type].keys()):
                options_tree.append(('model', options_type, None, name, add_options[name], ('???', None),'x'))
            elif name in list(help_options['params'][options_type].keys()):
                options_tree.append(('params', options_type, None, name, add_options[name], ('???', None),'x'))
            elif help_options['model'][options_type]['overwrite'][name][1] == 's':
                options_tree.append(('params', options_type, None, name, add_options[name], ('???', None),'x'))
            else:
                options_tree.append(('model', options_type, None, name, add_options[name], ('???', None),'x'))
    return options_tree, fixed_params
