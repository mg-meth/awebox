...
#['psi'] + struct_op.subkeys(model.variables, 'theta') * 2
    ...
    if 'ddl_t' in list(model.variables_dict['u'].keys()):
        ...
        power_updates[0] = ['dv_sq', 'dv_sq', 'psi'] + struct_op.subkeys(model.variables, 'theta') * 2
    # check if phase fix
    ...
    if 'ddl_t' in list(model.variables_dict['u'].keys()):
        ...
        nominal_landing_updates[0] = ['dv_sq', 'dv_sq', 'eta'] + struct_op.subkeys(model.variables, 'theta') * 2
    # check if phase fix
    if 'dl_t' in list(bounds_schedule.keys()):
    ...
    if 'ddl_t' in list(model.variables_dict['u'].keys()):
        ...
        transition_updates[0] = ['dv_sq', 'dv_sq', 'upsilon'] + struct_op.subkeys(model.variables, 'theta') * 2
    # check if phase fix
    if 'dl_t' in list(bounds_schedule.keys()):
...
def define_bound_update_schedule(model, nlp, formulation):
    ...
    if 'ddl_t' in list(model.variables_dict['u'].keys()):
        ...
        bound_schedule['dv_sq'][1] = ['lb', 'u', 'final']
        bound_schedule['dv_sq'][2] = ['ub', 'u', 'final']
